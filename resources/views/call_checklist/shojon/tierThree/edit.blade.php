...
<div class="row">
	<div class="col-md-10 mx-auto">
		<div class="tile">
			...
			<form id="myForm" action="{{ route('call_checklist.shojontierThree.update') }}" method="POST" role="form"
			...
			<div class="tile-body">
				...
				<div class="form-group">
					<label for="caller_name" class="form-label"><b>Client Name</b></label>
					<input type="text" class="form-control" name="client_name" readonly
					value="{{ $data->caller_name }}">
				</div>

				<div class="form-group">
					<label class="control-label" for="caller_id"><b>Client Id:</b></label><br>
					<input type="text" class="form-control" name="client_id" value="{{ $data->caller_id }}">
				</div>
...
<div class="form-group">
	...
	<div class="form-control">
		<div class="form-control">
			...
			<input type="text" class="form-control" name="examination_appearance" value="{{ $data->appearance }}">
		</div>
		<div class="form-control">
			<label class="control-label">Behavior (please explain)</label>
			<input type="text" class="form-control" name="examination_behavior" value="{{ $data->behavior }}">
		</div>
		<div class="form-control">
			<label class="control-label">Speech (please explain)</label>
			<input type="text" class="form-control" name="examination_speech" value="{{ $data->speech }}">
		</div>
		<div class="form-control">
			<label class="control-label">Mood and affect (please explain)</label>
			<input type="text" class="form-control" name="examination_affect" value="{{ $data->affect }}">
		</div>
		<div class="form-control">
			<label class="control-label">Thought (please explain)</label>
			<input type="text" class="form-control" name="examination_thought" value="{{ $data->thought }}">
		</div>
		<div class="form-control">
			<label class="control-label">Perception (please explain)</label>
			<input type="text" class="form-control" name="examination_perception" value="{{ $data->perception }}">
		</div>
		<div class="form-control">
			<label class="control-label">Cognition (please explain)</label>
			<input type="text" class="form-control" name="examination_cognition" value="{{ $data->cognition }}">
		</div>
		<div class="form-control">
			<label class="control-label">Insight & Judgement (please explain)</label>
			<input type="text" class="form-control" name="examination_judgement" value="{{ $data->judgement }}">
		</div>
	</div>
</div>
...
<div class="form-group">
	...
	<table class="table table-bordered border-primary" id="dynamic_field">
		...
		<tbody>
			...
			@if($key1 === $key2)
			<tr>
				<td><input type="text" name="Symptoms[]" value="{{$info}}"
					class="form-control name_list" /></td>
					<td><input type="text" name="Severity[]" value="{{$infoSev}}"
						class="form-control name_list" /></td>
	...
	<div class="form-group">
		...
		<textarea class="form-control" rows="3" name="illness_history">
			{{$data->problem_history}}
	...
	<div class="form-group">
		...
		<textarea class="form-control" rows="3" name="family_history">
			{{$data->family_history}}
	...
	<div class="form-group">
		...
		<textarea class="form-control" rows="3" name="birth_history">
			{{ $data->birth_history }}
	...
	<div class="form-group">
		...
		any</b></label>
		@if($data->birth_history != null)
		<textarea class="form-control" rows="3" name="substance_history">
			{{ $data->birth_history }}
...
<div class="form-group">
	...
	@if($data->previous_medication !="")
	<textarea class="form-control" rows="3" name="previous_medication">{{$data->previous_medication}}</textarea>
	@else
	<textarea class="form-control" rows="3" name="previous_medication"></textarea>
	@endif
</div>
...
<div class="form-group">
	...
	<div class="form-control @error('current_differential_diagnosis') is-invalid @enderror">
		...
		@foreach($types as $item)
		<input type="checkbox" name="current_differential_diagnosis[]" value="{{ $item}}"
		onclick="ShowCurrentDiagnosis()" />{{ $item }} <br>
		@endforeach
		@endif
...
<div class="form-group">
	...
	<textarea class="form-control" rows="3" name="prescribed_medications">
...
<div class="form-group">
	...
	<select name="psychotherapy_session_suggested" id="psychotherapy_session_suggested"
	list="suicidal_risk_list"
	class="form-control @error('psychotherapy_session_suggested') is-invalid @enderror">
	<datalist id="suicidal_risk_list">
		<option value="" selected disabled>Select Option</option>
		@foreach($types as $item)
		@if( $data->psychotherapy_session_suggested == $item)
		<option value="{{ $item }}" selected>{{ $item }}</option>
		@else
		<option value="{{ $item }}">{{ $item }}</option>
		@endif
...
<div class="form-group">
	...
	@php $types = ['Yes', 'No','Don’t Know']; @endphp
	<select name="client_ability_buy_medicine" id="client_ability_buy_medicine"
	list="suicidal_risk_list"
	class="form-control @error('client_ability_buy_medicine') is-invalid @enderror">
	<datalist id="suicidal_risk_list">
		<option value="" selected disabled>Select Option</option>
		@foreach($types as $item)
		@if( $data->client_ability_buy_medicine == $item)
		<option value="{{ $item }}" selected>{{ $item }}</option>
		@else
		<option value="{{ $item }}">{{ $item }}</option>
		@endif
...
<div class="form-group">
	...
	<select name="suitable_session_type" id="suitable_session_type" list="suicidal_risk_list"
	class="form-control @error('suitable_session_type') is-invalid @enderror">
	<datalist id="suicidal_risk_list">
		<option value="" disabled>Select Option</option>
		@foreach($types as $item)
		@if( $data->suitable_session_type == $item)
		<option value="{{ $item }}" selected>{{ $item }}</option>
		@else
		<option value="{{ $item }}">{{ $item }}</option>
		@endif
...
<div class="form-group">
	<label class="control-label" for="useful_effective"><b>21. How effective the client thinks the psychiatric consultation was </b></label><br>
	...
	<div class="form-control @error('useful_effective') is-invalid @enderror">
		...
		<input type="radio" name="useful_effective" value="{{ $item }}" checked="checked" />
		@else
...
<div class="form-control">
	...
	<div class="form-control @error('client_referral') is-invalid @enderror">
		...
		<input type="radio" name="client_referral" value="{{ $item }}" checked />
		@else
...
<div class="form-group">
	...
	<textarea rows="2" cols="50" class="form-control" name="next_session_plan"
	id="next_session_plan" value="{{ old('next_session_plan') }}">{{ $data->session_plan}}
</textarea>
...
<div class="form-group">
	...
	id="session_summary" value="{{ old('session_summary') }}">{{ $data->session_summary}}
</textarea>
</div>
...
<div class="form-group">
	...
	<div class="form-control @error('TreatmentGoal') is-invalid @enderror">
		...
		<div class="row">
			<div class="col">
				<input type="date" class="form-control" required name="next_session_date" value="{{$data->next_session_date}}">
			</div>
			<div class="col">
				<input type="time" class="form-control" required name="next_session_time" value="{{$data->next_session_time}}">
			</div>
		</div><br>
...
<div class="form-group">
	...
	<input type="text" class="form-control" name="physical_test" value="{{$data->physical_test}}">
</div>
...
@error('message') is-invalid @enderror" name="message" id="message" value="{{ $data->message }}"
